...
MongoDOWN.prototype._get = function (key, options, callback) {
  this._db[this.collection].findOne({ name : key }, function (err, doc) {
    if (err) return callback(err);
	if (!doc) return callback(new Error('notFound'));
	console.log(">>> MONGODOWN OPTIONS <<<");
	console.log(options);
    var value = options.asBuffer ?
      (Buffer.isBuffer(doc) ? doc : new Buffer(doc)) :
      (Buffer.isBuffer(doc) ? JSON.stringify(doc) : doc);
    console.log(">>> VALUE IN MONGODOWN <<<");
    console.log(value);
    callback(null, value);
  });
};
...
MongoDOWN.prototype._put = function (key, value, options, callback) {
  ...
  this._db[this.collection].update({ name : key }, data, { upsert: true }, callback);
};

MongoDOWN.prototype._del = function (key, options, callback) {
  this._db[this.collection].remove({ name: key }, callback);
};
